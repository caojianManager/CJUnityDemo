...
namespace CVR.InputField
{
        ...
        ILayoutElement
    {
        ...
        public void Backspace()
        {
            ...
            if (hasSelection)
            {
                LogText.instence.Log("Backspace1");
                Delete();
                UpdateTouchKeyboardFromEditChanges();
                SendOnValueChangedAndUpdateLabel();
            }
        ...
        {
            //当前是否正在输入。。。
            if (GetComponent<CVRInputFieldHander>().isKeycodeInputing())
            {
                Debug.LogError("取消选择 无效");
                //EventSystem.current.SetSelectedGameObject(gameObject, eventData);
                return;
            }
            Debug.LogError("取消选择 成功");
            DeactivateInputField();
